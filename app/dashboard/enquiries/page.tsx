...
function formatAddress(row: EnquiryRow) {
  return [row.address, row.area, row.city, row.state, row.pincode]
    .filter(Boolean)
    .join(", ");
}
...
export default function CourseEnquiriesPage() {
  ...
  const stats = useMemo(() => {
    ...
    return { total: allEnquiries.length, today, week };
  }, [allEnquiries]);
  ...
  const filtered = useMemo(() => {
    ...
    return rows;
  }, [allEnquiries, debouncedSearch, dateFilter]);
  ...
  const handleDelete = async (row: EnquiryRow) => {
    const result = await showDeleteConfirm(
      "Delete enquiry?",
      `Remove enquiry from ${row.fullName}? This cannot be undone.`
    );
    if (!result.isConfirmed) return;

    setDeletingId(row.id);
    try {
      const res = await fetch(`/api/course-enquiries?id=${row.id}`, {
        method: "DELETE",
        credentials: "include",
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
        showError("Error", json.error || "Failed to delete enquiry.");
        return;
      }
      showSuccess("Deleted", "Enquiry removed.");
  ...
  const exportCsv = useCallback(() => {
    ...
    a.href = url;
    a.download = `course-enquiries-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
        ...
        <div className="flex flex-col gap-4 px-5 py-4 sm:px-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0">
            <nav className="mb-1.5 flex flex-wrap items-center gap-1 text-[11px] text-white/55">
              ...
              <span>/</span>
              <span className="text-white/80">Course Enquiries</span>
            </nav>
            ...
            <p className="mt-1 max-w-xl text-xs text-white/60 sm:text-sm">
              Submissions from the user panel — also emailed to admin via SMTP
            </p>
          </div>
        ...
        <div className="flex flex-col gap-3 border-b border-[#1E4A85]/10 bg-gradient-to-r from-[#1E4A85]/[0.04] to-transparent px-4 py-3 sm:px-5">
          ...
          {(debouncedSearch || dateFilter !== "ALL") && (
            <p className="text-xs text-muted-foreground">
              Showing {total} of {stats.total} enquiries
              {debouncedSearch && (
                <button
                  type="button"
                  onClick={() => setSearch("")}
        ...
        <div className="overflow-x-auto">
          {isLoading && !data ? (
            ...
            <div className="px-6 py-16 text-center">
              ...
              <p className="text-lg font-bold text-foreground">No enquiries yet</p>
              <p className="mt-1 text-sm text-muted-foreground">
                {debouncedSearch || dateFilter !== "ALL"
                  ? "Try adjusting your filters."
                  : "Enquire Now submissions will appear here."}
              </p>
              <Link
                href="/userpanel"
                ...
                {items.map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span className="inline-flex items-center gap-1 rounded-full border border-[#1E4A85]/20 bg-[#1E4A85]/8 px-2 py-0.5 text-[11px] font-semibold text-[#1E4A85]">
            ...
            >
              ...
              <div className="flex-1 space-y-4 overflow-y-auto p-5">
                ...
                <section className="rounded-xl border border-border/60 p-4">
                  ...
                  <p className="font-semibold">{selected.courseName}</p>
                </section>
              ...
              <div className="flex gap-2 border-t border-border/60 p-4">
                <a
                  href={`mailto:${selected.email}?subject=Re: Course enquiry - ${encodeURIComponent(selected.courseName)}`}
