...
const UPLOAD_DIR = path.join(process.cwd(), "public", "uploads", "userpanel", "hero");
const PUBLIC_PREFIX = "/uploads/userpanel/hero/";

function isLocalHeroUrl(url: string | null | undefined): url is string {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const filename = `hero_${Date.now()}_${Math.random().toString(16).slice(2)}${safeExt}`;
    const absPath = path.join(UPLOAD_DIR, filename);

    const buf = Buffer.from(await file.arrayBuffer());
    await fs.writeFile(absPath, buf);

    // If admin replaced a previously uploaded local image, delete old one.
    if (isLocalHeroUrl(oldUrlStr) && oldUrlStr !== `${PUBLIC_PREFIX}${filename}`) {
    ...
    console.error("hero-background-image POST:", err);
