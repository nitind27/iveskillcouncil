...
const UPLOAD_DIR = path.join(process.cwd(), "public", "uploads", "userpanel", "about");
const PUBLIC_PREFIX = "/uploads/userpanel/about/";

function isLocalAboutUrl(url: string | null | undefined): url is string {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const filename = `about_${Date.now()}_${Math.random().toString(16).slice(2)}${safeExt}`;
    const absPath = path.join(UPLOAD_DIR, filename);

    const buf = Buffer.from(await file.arrayBuffer());
    await fs.writeFile(absPath, buf);

    if (isLocalAboutUrl(oldUrlStr) && oldUrlStr !== `${PUBLIC_PREFIX}${filename}`) {
    ...
    console.error("about-image POST:", err);
